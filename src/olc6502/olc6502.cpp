...
// It is 16x16 entries. This gives 256 instructions. It is arranged so that the bottom
...
void olc6502::reset()
{
	...
	addr_abs = 0xFFFC;
	uint16_t lo = read(addr_abs + 0);	//pc is 16bits, so two read operations (returning 8 bits each) are required
	uint16_t hi = read(addr_abs + 1);

	// Set it
	// This constructs the entire 16bit word from the lo and hi bits read above.
	// Shift left 8 bits results in 0xNN00, where NN represents hi byte
	// bitwise OR with lo results in 0xNNLL, where LL represents lo byte.
	pc = (hi << 8) | lo;
...
void olc6502::irq()
{
	...
	if (GetFlag(I) == 0)
	{
		...
		// The B, U and I flags are set at this point to indicate to the status register that an interrupt has occurred
		SetFlag(B, 0);
		SetFlag(U, 1);
		SetFlag(I, 1);
		...
		addr_abs = 0xFFFE;  // hard-coded address as defined by 6502 chip specification.
...
//
// To make a signed positive number negative, we can invert the bits and add 1 (called taking the 2's complement)
...
uint8_t olc6502::SBC()
{
	fetch(); //fetched is M in the equation above

	// Operating in 16-bit domain to capture carry out
	// We can invert the bottom 8 bits (or "low byte") with bitwise xor
	uint16_t value = ((uint16_t)fetched) ^ 0x00FF;			// What happened to the "+1" from above? we inverted the fetched data, but never did the +1 required for 2's complement

	// Notice this is exactly the same as addition from here!
	uint16_t temp = (uint16_t)a + value + (uint16_t)GetFlag(C); // A = A + ~M + C  but where is the +1??
...
uint8_t olc6502::AND()
{
	...
	SetFlag(Z, a == 0x00);
	SetFlag(N, a & 0x80);
	return 1;		// as per specification doc, this operation may require an extra clock cycle if a "page" is crossed
}
...
uint8_t olc6502::BCC()
{
	if (GetFlag(C) == 0)
	{
		...
		if ((addr_abs & 0xFF00) != (pc & 0xFF00)) //check for crossing page.
			cycles++;
...
uint8_t olc6502::BCS()
{
	if (GetFlag(C) == 1)
	{
		cycles++;		// branch directly modify the cycles variable. automatically adds 1 when branch is taken.
		addr_abs = pc + addr_rel;

		if ((addr_abs & 0xFF00) != (pc & 0xFF00))  //if branch needs to cross page boundary, incurs another +1 cycle "penalty"
			cycles++;
...
uint8_t olc6502::BRK()
{
	...
	SetFlag(I, 1);
	//the stack pointer is 16bits, so two write operations (8bits each) must be called.
	write(0x0100 + stkp, (pc >> 8) & 0x00FF);  // right shift 8 bits and bitwise AND the "lo" bite of the result to get the original "hi" byte adn write it
	stkp--;
	write(0x0100 + stkp, pc & 0x00FF);		// simply bitwise AND the "lo" byte to write the lo byte,
	stkp--;
